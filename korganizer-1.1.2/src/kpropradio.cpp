// $Id: kpropradio.cpp,v 1.5.2.2 1999/04/05 21:49:16 pbrown Exp $
...
//		debug("kpropradio: group=%s key=%s",ConfigObject->group(), Key.data() );
		if( Key.data() != 0 )
			ConfigObject->writeEntry( Key.data(), getState() );
		else debug("kpropradio: Null key not allowed");
	}
}
