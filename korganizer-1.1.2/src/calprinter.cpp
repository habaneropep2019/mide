 ...
 *
 * $Id: calprinter.cpp,v 1.23.2.1 1999/03/12 22:29:13 pbrown Exp $
 */
...
void CalPrinter::printTodo(const QDate &fd, const QDate &td)
{
  ...
  int count = 1;
  QString outStr;
  //  KDPEvent *currEvent(todoList.first());
			    ...
			    bool fullDate)
{
  ...
  int count = 1;
  QString outStr;
  KDPEvent *currEvent(eventList.first());
...
void CalPrinter::drawDay(QPainter &p, const QDate &qd, int width, int height)
{
  ...
  QList<KDPEvent> eventList = calendar->getEventsForDate(qd, TRUE);
  KDPEvent *currEvent;
				...
				int x, int y, int width, int height)
{
  ...
  // draw days of week
  for (int col = 0; col < 7; col++) {
    tmpStr.sprintf("%c",monthDate2.dayName(monthDate2.dayOfWeek())[0]);
